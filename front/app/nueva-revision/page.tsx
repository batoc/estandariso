...
export default function NuevaRevisionPage() {
  ...
  async function cargarDatosModulos() {
    try {
      const [
        ...
        pb.collection('compromisos').getFullList({ filter: 'estado = "pendiente"' }),
        pb.collection('gestion_cambios').getList(1, 6, { sort: '-fecha_identificacion' }),
        pb.collection('encuestas_satisfaccion').getList(1, 6, { sort: '-fecha_encuesta' }),
        pb.collection('quejas_reclamos').getList(1, 6, { sort: '-fecha_recepcion' }),
        pb.collection('objetivos_calidad').getFullList(),
        pb.collection('inspecciones_verificacion').getList(1, 6, { sort: '-created' }),
        pb.collection('no_conformidades').getList(1, 6, { sort: '-fecha_deteccion' }),
        pb.collection('indicadores').getFullList(),
        pb.collection('auditorias').getList(1, 6, { sort: '-fecha_auditoria' }),
        pb.collection('proveedores').getList(1, 6, { sort: '-fecha_evaluacion' }),
        pb.collection('recursos').getList(1, 6, { sort: '-fecha_evaluacion' }),
        pb.collection('riesgos_oportunidades').getList(1, 6, { sort: '-nivel_riesgo' }),
        pb.collection('oportunidades_mejora').getList(1, 6, { sort: '-fecha_identificacion' })
      ]);

      setCompromisosPrevios(resCompromisos || []);
      setCambios(resCambios.items || []);
      setEncuestas(resEncuestas.items || []);
      setQuejas(resQuejas.items || []);
      setObjetivos(resObjetivos || []);
      setInspecciones(resInspecciones.items || []);
      setNoConformidades(resNC.items || []);
      setIndicadores(resIndicadores || []);
      setAuditorias(resAuditorias.items || []);
      setProveedores(resProveedores.items || []);
      setRecursos(resRecursos.items || []);
      setRiesgos(resRiesgos.items || []);
      setMejoras(resMejoras.items || []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const revision = await pb.collection('revisiones').create({
      ...
      if (nuevosCompromisos.length > 0 && revision) {
        ...
        try {
          const promises = compromisosToInsert.map(c => pb.collection('compromisos').create(c));
          await Promise.all(promises);
        } catch (compromisosError) {
