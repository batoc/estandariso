...
export default function CompromisosPage() {
  ...
  const fetchCompromisos = async () => {
    setLoading(true);
    const data = await pb.collection('compromisos').getFullList({ sort: '-fecha_compromiso' });
    setCompromisos(data);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await pb.collection('compromisos').update(editingId, dataToSave);
        alert('Compromiso actualizado exitosamente');
        fetchCompromisos();
        resetForm();
      } else {
        await pb.collection('compromisos').create(dataToSave);
  ...
  const handleDelete = async (id: string) => {
    if (confirm('¿Eliminar este compromiso?')) {
      await pb.collection('compromisos').delete(id);
      fetchCompromisos();
    }
  };
