...
export default function MejorasPage() {
  ...
  async function cargarDatos() {
    try {
      const data = await pb.collection('oportunidades_mejora').getFullList({ sort: '-fecha_identificacion' });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        await pb.collection('oportunidades_mejora').update(editingId, dataToSave);
      } else {
        await pb.collection('oportunidades_mejora').create(dataToSave);
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('¿Eliminar esta oportunidad de mejora?')) {
      await pb.collection('oportunidades_mejora').delete(id);
        cargarDatos();
    }
  };
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Oportunidades de Mejora</h1>
          </div>
          <p className="text-sm text-slate-500 mt-1 ml-7">Gestión de la mejora continua (10.1, 10.3)</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="card p-4 border border-slate-200/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500">Total Mejoras</p>
              <p className="text-2xl font-bold text-slate-900">{mejoras.length}</p>
            </div>
        ...
        <div className="card p-4 border border-slate-200/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500">Implementadas</p>
              <p className="text-2xl font-bold text-slate-900">
        ...
        <div className="card p-4 border border-slate-200/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500">Eficaces</p>
              <p className="text-2xl font-bold text-slate-900">
