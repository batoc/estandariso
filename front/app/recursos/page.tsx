...
export default function RecursosPage() {
  ...
  const fetchRecursos = async () => {
    setLoading(true);
    const data = await pb.collection('recursos').getFullList({ sort: '-created' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await pb.collection('recursos').update(editingId, dataToSave);
      
        fetchRecursos();
        resetForm();
    } else {
      await pb.collection('recursos').create(dataToSave);
  ...
  const handleDelete = async (id: string) => {
    if (confirm('¿Eliminar recurso?')) {
      await pb.collection('recursos').delete(id);
        fetchRecursos();
    }
  };
    ...
    <div className="page-container">
      <div className="flex justify-between items-center mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Recursos</h1>
          </div>
          <p className="text-sm text-slate-500 mt-1 ml-7">Gestión de recursos (7.1)</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="card p-4 border border-slate-200/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">{recursos.length}</p>
            </div>
        ...
        <div className="card p-4 border border-slate-200/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500">Adecuados</p>
        ...
        <div className="card p-4 border border-slate-200/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500">Requieren Mejora</p>
              <p className="text-2xl font-bold text-slate-900">
            ...
            <table className="w-full">
              <thead className="bg-slate-50/80 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Capacidad</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Necesidad</th>
