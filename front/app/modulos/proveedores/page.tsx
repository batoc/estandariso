...
export default function ProveedoresPage() {
  ...
  const fetchProveedores = async () => {
    setLoading(true);
    const data = await pb.collection('proveedores').getFullList({ sort: 'nombre_proveedor' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await pb.collection('proveedores').update(editingId, dataToSave);
      
        fetchProveedores();
        resetForm();
    } else {
      await pb.collection('proveedores').create(dataToSave);
