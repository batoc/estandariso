...
export default function AuditoriasProveedoresPage() {
  ...
  async function cargarDatos() {
    try {
      const [auditoriasData, proveedoresData] = await Promise.all([
        pb.collection('auditorias_proveedores').getFullList({ sort: '-fecha_auditoria', expand: 'proveedor_id' }),
        pb.collection('proveedores').getFullList({ sort: 'nombre_proveedor' })
      ]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        await pb.collection('auditorias_proveedores').update(editingId, dataToSave);
      } else {
        await pb.collection('auditorias_proveedores').create(dataToSave);
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('¿Eliminar auditoría?')) {
      await pb.collection('auditorias_proveedores').delete(id);
        cargarDatos();
    }
  };
