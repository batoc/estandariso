...
export default function IndicadoresPage() {
  ...
  const fetchIndicadores = async () => {
    setLoading(true);
    try {
      const data = await pb.collection('indicadores').getFullList({ sort: '-created' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await pb.collection('indicadores').update(editingId, dataToSave);
      } else {
        await pb.collection('indicadores').create(dataToSave);
      }
      fetchIndicadores();
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('¿Eliminar este registro?')) {
      await pb.collection('indicadores').delete(id);
        fetchIndicadores();
    }
  };
